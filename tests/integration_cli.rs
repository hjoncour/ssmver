...
#[test]
fn bump_updates_cargo_lockfile() {
    let temp = TempDir::new().unwrap();
    init_git_repo(temp.path());
    fs::write(
        temp.path().join("Cargo.toml"),
    ...
    assert!(fs::read_to_string(temp.path().join("Cargo.toml"))
        .unwrap()
...
#[test]
fn bump_updates_tauri_project_versions() {
    let temp = TempDir::new().unwrap();
    init_git_repo(temp.path());
    fs::create_dir_all(temp.path().join("src")).unwrap();
    ...
    .unwrap();
    fs::write(temp.path().join("src-tauri/src/lib.rs"), "pub fn demo() {}\n").unwrap();
    fs::write(
    ...
    assert!(fs::read_to_string(temp.path().join("src-tauri/Cargo.toml"))
        .unwrap()
        .contains("version = \"1.3.0\""));
    assert!(fs::read_to_string(temp.path().join("src-tauri/tauri.conf.json"))
        .unwrap()
        .contains("\"version\": \"1.3.0\""));
}
