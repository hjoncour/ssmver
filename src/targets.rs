...
static RUBY_GEMSPEC_VERSION_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r#"(?m)^\s*spec\.version\s*=\s*(?:"(?P<value_dq>[^"\n]+)"|'(?P<value_sq>[^'\n]+)')\s*$"#,
    )
    .unwrap()
});
...
static DOTNET_INFO_VERSION_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"AssemblyInformationalVersion\((?:"(?P<value_dq>[^"]+)"|'(?P<value_sq>[^']+)')\)"#)
        .unwrap()
});
static XCODE_MARKETING_VERSION_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?m)^\s*MARKETING_VERSION\s*=\s*(?P<value_dq>[0-9]+\.[0-9]+(?:\.[0-9]+)?)\s*;"#)
        .unwrap()
});
static PLIST_BUNDLE_VERSION_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(
        r#"(?s)<key>CFBundleShortVersionString</key>\s*<string>(?P<value_dq>[^<]+)</string>"#,
    )
    .unwrap()
});
...
impl VersionTarget {
    ...
    pub fn is_blocking(&self) -> bool {
        matches!(
            self.status,
            TargetStatus::SkippedDynamic | TargetStatus::Error
        )
    }
}
...
pub fn discover_targets(repo_root: &Path, sync: &SyncSettings) -> Result<Vec<VersionTarget>> {
    ...
    for entry in walker {
        ...
        let relative = path
            ...
            .with_context(|| format!("failed to make {} relative to repo root", path.display()))?;
    ...
    targets.sort_by(|left, right| {
        left.path.cmp(&right.path).then_with(|| {
            format!("{:?}", left.target_kind).cmp(&format!("{:?}", right.target_kind))
        })
    });
    Ok(targets)
}

pub fn apply_version_to_targets(
    repo_root: &Path,
    ...
    for target in targets {
        ...
        let did_change = match target.target_kind {
            ...
            TargetKind::InfoPlist => update_info_plist(&repo_root.join(&target.path), new_version)?,
        };
...
fn detect_cargo_toml(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    let parsed = parse_consistent_versions(
        Ecosystem::Cargo,
        relative_path,
        TargetKind::CargoToml,
        authority,
        &versions,
    )?;
...
fn detect_package_json(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    let ecosystem = if json.get("engines").and_then(|e| e.get("vscode")).is_some()
        && json.get("publisher").is_some()
    {
        Ecosystem::Vscode
    } else {
        Ecosystem::Node
...
fn detect_npm_lockfile(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    Ok(vec![parse_consistent_versions(
        Ecosystem::Node,
        relative_path,
        TargetKind::NpmLockfile,
        TargetAuthority::Mirror,
        &versions,
    )?])
}
...
fn detect_maven_pom(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    let versions = [project_version, local_parent_version]
        ...
        .flatten()
        .collect::<Vec<_>>();
    Ok(vec![parse_consistent_versions_with_dynamic(
        Ecosystem::Maven,
        relative_path,
        TargetKind::MavenPom,
        TargetAuthority::LocalParent,
        &versions,
    )?])
}
...
fn detect_gradle_properties(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    if version_lines.len() > 1 {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::Error,
            None,
            "multiple version= assignments found",
        )]);
    }
    ...
    if version.contains("${") {
        return Ok(vec![VersionTarget::issue(
            ...
            Some(version.clone()),
            "dynamic gradle.properties version is not supported",
        )]);
    }
...
fn detect_gradle_build_script(
    repo_root: &Path,
    relative_path: &Path,
) -> Result<Vec<VersionTarget>> {
    let sibling_properties = relative_path
        .parent()
    ...
    if repo_root.join(&sibling_properties).is_file() {
        ...
        if content
            .lines()
            .any(|line| line.trim_start().starts_with("version") && line.contains('='))
        {
            return Ok(Vec::new());
        }
    }
    ...
    if generic_matches > literal_matches.len() {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::SkippedDynamic,
            None,
            "dynamic or unsupported version assignment found in Gradle build script",
        )]);
    }
    if literal_matches.len() != 1 {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::Error,
            None,
            "expected exactly one top-level literal version assignment",
        )]);
    }
...
fn detect_pyproject_toml(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    if dynamic_array_contains_version(get_item(&doc, &["project", "dynamic"])) {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::SkippedDynamic,
            None,
            "pyproject.toml declares dynamic project.version",
        )]);
    }
    ...
    let versions = [project_version, poetry_version]
        .into_iter()
        .flatten()
        .collect::<Vec<_>>();
    Ok(vec![parse_consistent_versions(
...
fn detect_setup_cfg(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    if value.contains('%') || value.contains('{') {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::SkippedDynamic,
            Some(value),
            "dynamic setup.cfg metadata.version is not supported",
        )]);
    }
...
fn detect_python_dunder_version(
    repo_root: &Path,
    relative_path: &Path,
) -> Result<Vec<VersionTarget>> {
    let path = repo_root.join(relative_path);
    let content = fs::read_to_string(&path)?;
    let literal_matches = PYTHON_DUNDER_VERSION_RE
        .captures_iter(&content)
        .map(|captures| capture_regex_value(&captures).unwrap().to_string())
        .collect::<Vec<_>>();
    let generic_matches = Regex::new(r"(?m)^__version__\s*=")
        .unwrap()
        .find_iter(&content)
        .count();
    ...
    if generic_matches > literal_matches.len() {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::SkippedDynamic,
            None,
            "dynamic __version__ assignment is not supported",
        )]);
    }
    if literal_matches.len() != 1 {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::Error,
            None,
            "expected exactly one literal __version__ assignment",
        )]);
    }
...
fn detect_dotnet_project(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
        ...
        .any(|value| value.contains("$(") || value.contains("@("))
    {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::SkippedDynamic,
            None,
            "dynamic <Version> value is not supported",
        )]);
    }
...
fn detect_dotnet_assembly_info(
    repo_root: &Path,
    relative_path: &Path,
) -> Result<Vec<VersionTarget>> {
    let path = repo_root.join(relative_path);
    ...
    if let Some(value) = assembly.as_deref() {
        normalized.push(normalize_dotnet_numeric_version(value).ok_or_else(|| {
            anyhow!(
                "unsupported AssemblyVersion format in {}",
    ...
    if let Some(value) = file.as_deref() {
        normalized.push(normalize_dotnet_numeric_version(value).ok_or_else(|| {
            anyhow!(
                "unsupported AssemblyFileVersion format in {}",
                relative_path.display()
            )
        })?);
    }
...
fn detect_ruby_gemspec(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    if literal_matches.len() != 1 {
        return Ok(vec![VersionTarget::issue(
            ...
            TargetStatus::Error,
            None,
            "expected exactly one literal gemspec version assignment",
        )]);
    }
...
fn detect_xcode_project(repo_root: &Path, relative_path: &Path) -> Result<Vec<VersionTarget>> {
    ...
    if unique.len() > 1 {
        return Ok(vec![VersionTarget::issue(
            ...
            None,
            format!(
                "conflicting MARKETING_VERSION values: {}",
                unique.join(", ")
            ),
        )]);
    }
...
fn update_npm_lockfile(
    repo_root: &Path,
    relative_path: &Path,
    all_targets: &[VersionTarget],
    new_version: &Version,
) -> Result<bool> {
    let path = repo_root.join(relative_path);
    let content = fs::read_to_string(&path)?;
    ...
    if root.contains_key("version") {
        root.insert(
            "version".to_string(),
            JsonValue::String(version_string.clone()),
        );
        changed = true;
    }
    ...
    if let Some(packages) = root.get_mut("packages").and_then(JsonValue::as_object_mut) {
        ...
        for target in all_targets
            .iter()
            .filter(|target| target.target_kind == TargetKind::PackageJson)
        {
            ...
            if let Ok(stripped) = package_path.strip_prefix(lock_dir) {
                ...
                if let Some(package_entry) =
                    packages.get_mut(&key).and_then(JsonValue::as_object_mut)
                {
...
fn update_dotnet_assembly_info(path: &Path, new_version: &Version) -> Result<bool> {
    ...
    updated = DOTNET_ASSEMBLY_VERSION_RE
        .replace_all(&updated, format!("AssemblyVersion(\"{numeric}\")"))
        .into_owned();
    updated = DOTNET_FILE_VERSION_RE
        .replace_all(&updated, format!("AssemblyFileVersion(\"{numeric}\")"))
        .into_owned();
    updated = DOTNET_INFO_VERSION_RE
        .replace_all(
            &updated,
            format!("AssemblyInformationalVersion(\"{informational}\")"),
        )
        .into_owned();
    if updated == content {
        return Ok(false);
...
fn update_xcode_project(path: &Path, new_version: &Version) -> Result<bool> {
    let content = fs::read_to_string(path)?;
    let version_string = new_version.to_string();
    let updated = XCODE_MARKETING_VERSION_RE
...
fn update_info_plist(path: &Path, new_version: &Version) -> Result<bool> {
    let content = fs::read_to_string(path)?;
    let version_string = new_version.to_string();
    let updated = PLIST_BUNDLE_VERSION_RE
        .replace_all(&content, |captures: &Captures<'_>| {
            let whole = captures.get(0).unwrap().as_str();
            let matched = captures.name("value_dq").unwrap().as_str();
            whole.replacen(matched, &version_string, 1)
...
fn update_json_version_field<F>(
    path: &Path,
    new_version: &Version,
    should_manage: F,
) -> Result<bool>
...
fn parse_consistent_versions(
    ecosystem: Ecosystem,
    path: &Path,
    target_kind: TargetKind,
    authority: TargetAuthority,
    raw_versions: &[String],
) -> Result<VersionTarget> {
    parse_consistent_versions_with_dynamic(ecosystem, path, target_kind, authority, raw_versions)
}

fn parse_consistent_versions_with_dynamic(
    ecosystem: Ecosystem,
    path: &Path,
    target_kind: TargetKind,
    authority: TargetAuthority,
    ...
    if raw_versions.iter().any(|value| looks_dynamic(value)) {
        return Ok(VersionTarget::issue(
            ecosystem,
            path,
            target_kind,
            authority,
            TargetStatus::SkippedDynamic,
            raw_versions.first().cloned(),
            "dynamic version expression is not supported",
        ));
    }
    ...
    match parsed {
        Ok(parsed) => {
            ...
            if unique.len() > 1 {
                return Ok(VersionTarget::issue(
                    ...
                    TargetStatus::Error,
                    unique.first().cloned(),
                    format!("file contains conflicting versions: {}", unique.join(", ")),
                ));
            }

            Ok(VersionTarget::managed(
                ecosystem,
                path,
                target_kind,
                authority,
        ...
        Err(error) => Ok(VersionTarget::issue(
            ecosystem,
            path,
            target_kind,
            authority,
...
fn parse_version(
    ...
    Version::parse(raw)
        .map_err(|error| {
            anyhow!(
                "{:?} {:?} in {} has invalid semver `{raw}`: {error}",
                ecosystem,
                target_kind,
                path.display()
            )
